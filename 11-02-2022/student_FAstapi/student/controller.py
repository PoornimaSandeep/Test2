...
from service import service_insert, service_retrive, \
    service_retrive_one, service_update,service_delete,service_subject,service_csv
import models
...
@app.post('/uploadfile')
async def retrieve_csv(data_file:UploadFile,db : Session = Depends(get_db)):
